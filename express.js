...
io.sockets.on('connection',function(socket) {
	...
	socket.on("loadSalon", function(salon) {
		console.log(salon);
		if (salon == "MIAW") {
			socket.broadcast.emit("users", users_MIAW);
		} else if(salon == "ASSR") {
	...
	socket.on("start", function() {
		connection.connect(function(err) {
			data = [];
			// Gets the questions
			var query1 = connection.query("SELECT * FROM question", function(err, ques) {
				if (err) throw err;
				data.push(ques);
			});
			// Gets the possible answers
			var query2 = connection.query("SELECT * FROM reponse", function(err, ans) {
				if (err) throw err;
				data.push(ans);
			});
			// Gets all the right answers to all questions
			var query3 = connection.query("SELECT * FROM question_reponse", function(err, ques_ans) {
				if(err) throw err;
				data.push(ques_ans);
			});
		});
